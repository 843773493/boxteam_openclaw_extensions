...
- `speakMode`：speak 转发模式，可选，`batch` 表示等完整回复后一次性转发，`stream` 表示收到回复块时立即转发，默认 `batch`
...
1. 先执行本地端到端启动脚本，完成 OpenClaw gateway 停止、插件升级安装和 gateway 启动等待：

```bat
node ./scripts/agent/bootstrap-local-e2e.mjs
```
